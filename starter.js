...
//creating a higher-order function that will take the above functions as callback. its gonna check and convert if the arguments received are numbers, then involve the callback function
...
const result = calculator(3,2,multiply)
console.log(result)
...
//creates a function that lets us apply discount based off of our inventory

const applyDiscountByInventory = (arr,callback,discount,amount) => {
    arr.forEach(product => {
...
applyDiscountByInventory(dogProducts,applyflatRateDiscount,2,80)
console.log(dogProducts)
    ...
    return function(ingredients){
        ...
        for (let i=0;i< ingredients.length; i++){
            if(ingredients.length === 1){
                ...
                order += `${ingredients[i]},`
                //same as: order = order + `${ingredients[i]}`
            }
        } return order
    }
}

const makePumpernickelsanwitch = makeSandwich('pumpernickle')

console.log(makePumpernickelsanwitch(['cream-cheese', 'peppers','capicola', 'seasoning']))
